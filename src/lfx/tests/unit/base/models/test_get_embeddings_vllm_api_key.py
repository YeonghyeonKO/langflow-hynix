...
``get_llm()`` already re-resolves ``VLLM_API_KEY`` for vLLM Language, but ``get_embeddings()``
had no such guard. These tests pin the fixed behavior:
...
def _run(provider_vars: dict[str, str]) -> dict:
    """Instantiate embeddings for vLLM Embedding and return the captured kwargs.
